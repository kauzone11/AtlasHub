...
import { approveHubWalletRequest } from "@/lib/hub/wallet-operations";
...
export const POST = withHubApi<Context>(async (request, context) => {
  ...
  const result = await prisma.$transaction((tx) => approveHubWalletRequest(tx, { requestId: id, organizationId: session.organizationId, actorId: session.memberId, reviewNote }), { isolationLevel: "Serializable" });
