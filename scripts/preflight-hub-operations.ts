...
async function main() {
  ...
  const applied = state[0]?.migration_table ? await prisma.$queryRaw<Array<{ applied: boolean }>>`
    SELECT EXISTS (SELECT 1 FROM "_prisma_migrations" WHERE migration_name=${migrationName} AND finished_at IS NOT NULL AND rolled_back_at IS NULL) AS applied` : [{ applied: false }];
  if (applied[0]?.applied) { console.log(`[hub-operations-preflight] migration=${migrationName} applied=true`); console.log("[hub-operations-preflight] incompatible_total=0"); return; }
